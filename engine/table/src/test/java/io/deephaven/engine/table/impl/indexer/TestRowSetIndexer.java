...
public class TestRowSetIndexer extends RefreshingTableTestCase {
    ...
    public void testGrouping(final boolean immutableColumns, final Random random, final MutableInt numSteps) {
        ...
        final EvalNugget[] en = new EvalNugget[] {
                EvalNugget.from(() -> UpdateContext.updateGraphProcessor().exclusiveLock().computeLocked(
                        () -> queryTable.head(0))),
                EvalNugget.from(() -> UpdateContext.updateGraphProcessor().exclusiveLock().computeLocked(
                        () -> queryTable.head(1))),
                EvalNugget.from(() -> UpdateContext.updateGraphProcessor().exclusiveLock().computeLocked(
        ...
        Table by = UpdateContext.updateGraphProcessor().exclusiveLock().computeLocked(
                () -> queryTable.avgBy("Sym"));
        addGroupingValidator(by, "groupBy");
        Table avgBy = UpdateContext.updateGraphProcessor().exclusiveLock().computeLocked(
                () -> queryTable.avgBy("Sym"));
        addGroupingValidator(avgBy, "avgBy");
        Table avgBy1 = UpdateContext.updateGraphProcessor().exclusiveLock().computeLocked(
                () -> queryTable.avgBy("Sym", "intCol"));
        addGroupingValidator(avgBy1, "avgBy1");

        Table merged = Require.neqNull(UpdateContext.updateGraphProcessor().exclusiveLock().computeLocked(
                () -> TableTools.merge(queryTable)), "TableTools.merge(queryTable)");
        addGroupingValidator(merged, "merged");
        Table updated = UpdateContext.updateGraphProcessor().exclusiveLock()
