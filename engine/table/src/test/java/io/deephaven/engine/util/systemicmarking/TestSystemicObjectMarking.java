...
public class TestSystemicObjectMarking extends RefreshingTableTestCase {
    public void testSystemicObjectMarking() {
        ...
        final Table updated = UpdateContext.updateGraphProcessor().sharedLock().computeLocked(() -> source.update("UC=Str.toUpperCase()"));
        final Table updated2 = SystemicObjectTracker.executeSystemically(false, () -> UpdateContext.updateGraphProcessor().sharedLock().computeLocked(() -> source.update("LC=Str2.toLowerCase()")));
