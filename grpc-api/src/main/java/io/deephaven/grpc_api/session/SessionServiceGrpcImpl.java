...
public class SessionServiceGrpcImpl extends SessionServiceGrpc.SessionServiceImplBase {
    ...
    @Override
    public void closeSession(final HandshakeRequest request, final StreamObserver<CloseSessionResponse> responseObserver) {
        GrpcUtil.rpcWrapper(log, responseObserver, () -> {
            if (request.getAuthProtocol() != 0) {
                responseObserver.onError(GrpcUtil.statusRuntimeException(Code.INVALID_ARGUMENT, "Protocol version not allowed."));
                return;
    ...
    public void release(final ReleaseRequest request, final StreamObserver<ReleaseResponse> responseObserver) {
        GrpcUtil.rpcWrapper(log, responseObserver, () -> {
            final SessionState session = service.getCurrentSession();
            if (!request.hasId()) {
                responseObserver.onError(GrpcUtil.statusRuntimeException(Code.INVALID_ARGUMENT, "Release ticket not supplied"));
                return;
            }
            ...
            synchronized (export) {
                ...
                if (currState != ExportNotification.State.PENDING
                    ...
                    responseObserver.onError(GrpcUtil.statusRuntimeException(Code.NOT_FOUND, "Ticket already in state: " + currState));
                    return;
                }
            }
    ...
    public void exportFromTicket(ExportRequest request, StreamObserver<ExportResponse> responseObserver) {
        GrpcUtil.rpcWrapper(log, responseObserver, () -> {
            ...
            if (!request.hasSourceId()) {
                responseObserver.onError(GrpcUtil.statusRuntimeException(Code.INVALID_ARGUMENT, "Source ticket not supplied"));
                return;
            }
            if (!request.hasResultId()) {
                responseObserver.onError(GrpcUtil.statusRuntimeException(Code.INVALID_ARGUMENT, "Result ticket not supplied"));
                return;
            }
