...
public class ApplicationTicketResolver extends TicketResolverBase {
    ...
    @Override
    public <T> SessionState.ExportObject<T> resolve(final @Nullable SessionState session, final Flight.FlightDescriptor descriptor) {
    ...
    private AppFieldId appFieldIdFor(final ByteBuffer ticket) {
        ...
        if (app == null) {
            throw GrpcUtil.statusRuntimeException(Code.NOT_FOUND, "No application exists with the identifier: " + appId);
        }
    ...
    private AppFieldId appFieldIdFor(final Flight.FlightDescriptor descriptor) {
        ...
        if (descriptor.getPathCount() != 4) {
            throw GrpcUtil.statusRuntimeException(Code.FAILED_PRECONDITION,
                    "Cannot parse descriptor: unexpected path length (found: " + TicketRouterHelper.getLogNameFor(descriptor) + ", expected: 4)");
        }
