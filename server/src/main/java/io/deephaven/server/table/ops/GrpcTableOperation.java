...
public abstract class GrpcTableOperation<T> {
    ...
    /**
     * Get the source tables for this operation.
     *
     * @param sourceTables the export object wrapped tables which are guaranteed to be satisfied
     * @return the unwrapped source tables for this operation
     */
    protected List<Table> toTables(final List<SessionState.ExportObject<Table>> sourceTables) {
        return sourceTables.stream().map(SessionState.ExportObject::get).collect(Collectors.toList());
    }
}
