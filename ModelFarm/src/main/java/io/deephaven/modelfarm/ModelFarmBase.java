...
public abstract class ModelFarmBase<DATATYPE> implements ModelFarm {
    ...
    protected static FunctionalInterfaces.ThrowingBiConsumer<QueryDataRetrievalOperation, Table, RuntimeException> getDoLockedConsumer(
        ...
        switch (lockType) {
            ...
            case UGP_LOCK:
                return (queryDataRetrievalOperation, source) -> UpdateContext.updateGraphProcessor().exclusiveLock()
                        .doLocked(() -> queryDataRetrievalOperation.retrieveData(false));
            case UGP_READ_LOCK:
                return (queryDataRetrievalOperation, source) -> UpdateContext.updateGraphProcessor().sharedLock()
                        .doLocked(() -> queryDataRetrievalOperation.retrieveData(false));
            case SNAPSHOT:
