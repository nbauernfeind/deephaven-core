 ...
 * It does not require any pre-sorting of the input table, instead preserving relative order in the initial output and
 * each subsequent refresh.
 * Relative to SortedClockFilter, this implementation may require less overall storage and do less overall work for
 * tables with relatively few monotonically nondecreasing ranges (that is, m (number of ranges) &lt;&lt;&lt; n (size in rows)),
 * but it will do more work on refresh().
 */
public class UnsortedClockFilter extends ClockFilter {
    ...
    private class RangeComparator implements Comparator<Range> {
        ...
        public int compare(final Range r1, final Range r2) {
            ...
            return DBLanguageFunctionUtil.compareTo(nanosColumnSource.getLong(r1.nextKey), nanosColumnSource.getLong(r2.nextKey));
        }
    }
    ...
    protected Index updateAndGetAddedIndex() {
        ...
        while ((nextRange = rangesByNextTime.peek()) != null && (resultBuilder = nextRange.consumeKeysAndAppendAdded(nanosColumnSource, nowNanos, addedBuilder)) != null) {
