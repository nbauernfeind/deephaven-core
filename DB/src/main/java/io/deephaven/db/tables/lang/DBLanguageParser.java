...
public final class DBLanguageParser extends GenericVisitorAdapter<Class<?>, DBLanguageParser.VisitArgs> {
    ...
    private static final Set<String> simpleNameWhiteList = Collections.unmodifiableSet(new HashSet<>(Arrays.asList("java.lang", DbArrayBase.class.getPackage().getName())));
    ...
    /**
     * Create a DBLanguageParser and parse the given {@code expression}. After construction,
     * the {@link DBLanguageParser.Result result} of parsing the {@code expression} is available with the
     ...
     * @param staticImports Wildcard static imports. All static variables and methods for the given classes are imported.
    ...
    public DBLanguageParser(String expression,
        ...
        this(expression,packageImports,classImports,staticImports,variables,variableParameterizedTypes,true);
    }
    ...
    public DBLanguageParser(String expression,
                            ...
                            Collection<Class<?>> staticImports,
                            Map<String, Class<?>> variables,
                            Map<String, Class<?>[]> variableParameterizedTypes, boolean unboxArguments) throws QueryLanguageParseException {
        this.packageImports = packageImports==null ? Collections.emptySet() : Require.notContainsNull(packageImports, "packageImports");
        this.classImports = classImports==null ? Collections.emptySet() : Require.notContainsNull(classImports, "classImports");
        this.staticImports = staticImports==null ? Collections.emptySet() : Require.notContainsNull(staticImports, "staticImports");
        this.variables = variables==null ? Collections.emptyMap() : variables;
        this.variableParameterizedTypes = variableParameterizedTypes ==null ? Collections.emptyMap() : variableParameterizedTypes;
        ...
        try {
            ...
            QueryLanguageParseException newException = new QueryLanguageParseException(exceptionMessageBuilder.toString());
    ...
    /**
     * Retrieves the result of the parser, which includes the translated expression,
     * its return type, and the variables it uses.
     */
    ...
    /**
     * Convert single equals signs (the assignment operator) to double-equals signs (equality operator).
     * The parser will then replace the equality operator with an appropriate equality-checking methods.
     * Assignments are not supported.
     *
     * This method does not have any special handling for backticks; accordingly this
     * method should be run <b>after</b> {@link #convertBackticks(String)}.
     *
     ...
     *
     * Also, within backticks, double-quotes are automatically re-escaped. For example, in the following string
    ...
    /**
     * Search for a class with the given {@code name}. This can be a fully-qualified name,
     * or the simple name of an imported class.
    ...
    private Method getMethod(final Class<?> scope, final String methodName, final Class<?>[] paramTypes, final Class<?>[][] parameterizedTypes){
    ...
    private Class<?> calculateMethodReturnTypeUsingGenerics(Method method, Class<?>[] paramTypes, Class<?>[][] parameterizedTypes){
    ...
    private Constructor<?> getConstructor(final Class<?> scope, final Class<?>[] paramTypes, final Class<?>[][] parameterizedTypes){
        ...
        if (acceptableConstructors.size()==0){
            throw new RuntimeException("Cannot find constructor for " + scope.getName() + '(' + paramsTypesToString(paramTypes) + ')' + (scope!=null ? " in "+scope : ""));
        }
    ...
    private static <EXECUTABLE_TYPE extends Executable> void possiblyAddExecutable(final List<EXECUTABLE_TYPE> accepted,
        ...
        if (candidate.getName().equals(name)){
            ...
            if (candidate.isVarArgs() ? candidateParamTypes.length>paramTypes.length+1 : candidateParamTypes.length!=paramTypes.length){
                return;
            }

            boolean acceptable=true;

            for (int i=0; i<(candidate.isVarArgs() ? candidateParamTypes.length-1 : candidateParamTypes.length); i++){
                Class<?> paramType=paramTypes[i];
            ...
            if (candidate.isVarArgs() && paramTypes.length >= candidateParamTypes.length){
                ...
                if (isDbArray(paramType) && candidateParamTypes[candidateParamTypes.length-1].isArray()){
                    paramType=convertDBArray(paramType, parameterizedTypes[candidateParamTypes.length-1]==null ? null : parameterizedTypes[candidateParamTypes.length-1][0]);
                }
    ...
    private static <EXECUTABLE_TYPE extends Executable> Boolean isMoreSpecificExecutable(final EXECUTABLE_TYPE e1, final EXECUTABLE_TYPE e2) {
        ...
        if (!Arrays.equals(e1ParamTypes, e2ParamTypes)){  //this means that e2 params are more specific
            return true;
        }
    ...
    static boolean isNonFPNumber(Class<?> type){
        ...
        return type==int.class || type==long.class || type==byte.class || type==short.class || type==char.class;
    }
    ...
    /**
     * Converts the provided argument {@code expressions} for the given {@code executable} so that the expressions
     * whose types (expressionTypes) do not match the corresponding declared argument types ({@code argumentTypes})
     * may still be used as arguments.
     *
     ...
     * @return An array of new expressions that maintain the 'meaning' of the input {@code expressions} but are appropriate
     * to pass to {@code executable}
     */
    private Expression[] convertParameters(final Executable executable, final Class<?>[] argumentTypes, final Class<?>[] expressionTypes, final Class<?>[][] parameterizedTypes, Expression[] expressions){
        final int nArgs = argumentTypes.length; // Number of declared arguments
        for (int ai = 0; ai<(executable.isVarArgs() ? nArgs -1 : nArgs); ai++){
            if (argumentTypes[ai]!=expressionTypes[ai] && argumentTypes[ai].isPrimitive() && expressionTypes[ai].isPrimitive()){
                expressions[ai]=new CastExpr(new PrimitiveType(PrimitiveType.Primitive.valueOf(StringUtils.makeFirstLetterCapital(argumentTypes[ai].getSimpleName()))), expressions[ai]);
            }
        ...
        if (executable.isVarArgs()){
            ...
            if(nArgExpressions == nArgs
                ...
                for (int ei = nArgs - 1; ei < nArgExpressions; ei++) { // iterate over the vararg argument expresions
                    if (varArgType != expressionTypes[ei] && varArgType.isPrimitive() && expressionTypes[ei].isPrimitive()) {  // cast primitives to the appropriate type
                        expressions[ei] = new CastExpr(new PrimitiveType(PrimitiveType.Primitive.valueOf(StringUtils.makeFirstLetterCapital(varArgType.getSimpleName()))), expressions[ei]);
                    }
            ...
            if (varArgType.isPrimitive() && anyExpressionTypesArePrimitive){        //we have some problems with ambiguous oddities and varargs, so if its primitive lets just box it ourselves
    ...
    public Class<?> visit(NameExpr n, VisitArgs printer) {
        ...
        Our parser doesn't work exactly this way (some cases are not relevant, and the work
        is split between this class and the parser library), but the behavior should be
        consistent with the spec.

        What matters here:
        1)  If it's a simple name (i.e. not a qualified name; doesn't contain a '.'), then
                1. Check whether it's in the scope
                2. If it's not in the scope, see if it's a static import
                3. If it's not a static import, then it's not a situation the DBLanguageParser
                   has to worry about.
        2)  Qualified names -- we just throw them to 'findClass()'. Many details are not
    ...
    public Class<?> visit(ArrayAccessExpr n, VisitArgs printer) {
        /*
            ...
            The "Price_[i-1]" is translated to "Price.get(i-1)". But we do this generically, not just for DbArrays.
            As an example, this works (column Blah will be set to "hello"):

                map = new HashMap();
                map.put("a", "hello")
                t = emptyTable(1).update("Blah=map[`a`]")
    ...
    public Class<?> visit(CastExpr n, VisitArgs printer) {
        ...
        final Class<?> unboxedExprType = !fromBoxedType ? null : io.deephaven.util.type.TypeUtils.getUnboxedType(exprType);
        ...
        if (toPrimitive) { // Casting to a primitive
            /*
             * booleans can only be cast to booleans, and only booleans can be cast to booleans.
             * See table 5.5-A at the link above.
             *
            ...
            // Now check whether we're converting from a boxed type
                ...
                if (!ret.equals(unboxedExprType) && // Unboxing and Identity conversions are always OK
        ...
        /*
        When casting primitives to boxed types, only boxing conversions are allowed
        When casting boxed types to boxed types, only the identity conversion is allowed
        */
        else {
            if (io.deephaven.util.type.TypeUtils.isBoxedType(ret) && (fromPrimitive || fromBoxedType) && !(ret.equals(io.deephaven.util.type.TypeUtils.getBoxedType(exprType)))) {
                throw new RuntimeException("Incompatible types; " + exprType.getName() +
                        " cannot be converted to " + ret.getName());
            }
            isWidening = false;
        ...
        /*
        Now actually print the cast. For casts to primitives (except boolean),
        we use special null-safe functions (e.g. intCast()) to perform the cast.

        There is no "booleanCast()" function.

        There are also no special functions for the identity conversion -- e.g. "intCast(int)"
        */
        if (toPrimitive && !ret.equals(boolean.class) && !ret.equals(exprType)) { // Casting to a primitive, except booleans and the identity conversion
            printer.append(ret.getSimpleName());
            printer.append("Cast(");

            /*
            When unboxing to a wider type, do an unboxing conversion followed by a widening conversion.
            See table 5.5-A in the JLS, at the link above.
             */
            if(isWidening) {
    ...
    /**
     * Checks whether the conversion from {@code original} to {@code target} is a widening primitive conversion.
     * The arguments must be primitive types (not boxed types).
     *
     * This method return false if {@code original} and {@code target} represent the same type, as such a
     * conversion is the identity conversion, not a widening conversion.
     *
     ...
     * @return {@code true} if a conversion from {@code original} to {@code target} is a widening conversion;
    ...
    static boolean isWideningPrimitiveConversion(Class<?> original, Class<?> target) {
        ...
        switch (originalEnum) {
            case BytePrimitive:
                if(target == short.class) return true;
            case ShortPrimitive:
            case CharPrimitive:
                if(target == int.class) return true;
            case IntPrimitive:
                if(target == long.class) return true;
            case LongPrimitive:
                if(target == float.class) return true;
            case FloatPrimitive:
                if(target == double.class) return true;
        }
        return false;
    }
    ...
    private enum DBLanguageParserPrimitiveType {
        ...
        private static final Map<Class<?>, DBLanguageParserPrimitiveType> primitiveClassToEnumMap = Stream.of(DBLanguageParserPrimitiveType.values())
    ...
    public Class<?> visit(FieldAccessExpr n, VisitArgs printer) {
        ...
        if (scopeExpr instanceof FieldAccessExpr
                && (scopeType = findClass(scopeName)) != null) { // 'scope' was a class, and we found it - print 'scopeType' ourselves
    ...
    public Class<?> visit(IntegerLiteralExpr n, VisitArgs printer) {
        ...
        /* In java, you can't compile if your code contains an integer literal that's too big to fit in an int.
         * You'd need to add an "L" to the end, to indicate that it's a long.
         *
         * But in the DB, we assume you don't mind extra precision and just want your query to work, so when an 'integer'
         * literal is too big to fit in an int, we automatically add on the "L" to promote the literal from an int to a long.
         *
    ...
    public Class<?> visit(ClassExpr n, VisitArgs printer) {
        ...
        // TODO (NATE: NOCOMMIT): VERIFY THIS WAS A BUG
        return type;
    }
