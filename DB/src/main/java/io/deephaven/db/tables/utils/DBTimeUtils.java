...
public class DBTimeUtils {
    ...
    private static final Pattern STD_DATE_PATTERN2 =
            Pattern.compile("^(?<year>[0-9][0-9][0-9][0-9])(?<month>[0-9][0-9])(?<day>[0-9][0-9])$");
    /** Matches variations of month/day/year or day/month/year or year/month/day
     * - how this is interpreted depends on the DBTimeUtils.dateStyle system property.
     */
    private static final Pattern SLASH_DATE_PATTERN =
            Pattern.compile("^(?<part1>[0-9]?[0-9](?<part1sub2>[0-9][0-9])?)\\/(?<part2>[0-9]?[0-9])\\/(?<part3>[0-9]?[0-9](?<part3sub2>[0-9][0-9])?)$");
    ...
    /** for LocalTime literals. Note these must begin with "L" to avoid ambiguity with the older TIME_AND_DURATION_PATTERN */
    ...
    private static final boolean ENABLE_MICROTIME_HACK = Configuration.getInstance().getBooleanWithDefault("DBTimeUtils.enableMicrotimeHack", false);
    ...
    private static final DateStyle DATE_STYLE = DateStyle.valueOf(Configuration.getInstance().getStringWithDefault("DBTimeUtils.dateStyle", DateStyle.MDY.name()));
    ...
    private enum DateGroupId {
        ...
        DateGroupId(int id, ChronoField field) { this.id = id; this.field = field; }
    }
    ...
    /**
     * Allows setting an alternate date instead of "today" to be returned from {@link #currentDateNy}.
    ...
    /**
     * Allows setting a custom time provider instead of actual current time.
     * This is mainly used when setting up for a replay simulation.
     */
     ...
     * @return A long value of milliseconds since Epoch, or a {@link QueryConstants#NULL_LONG} value if the {@link DBDateTime} is null.
     */
     ...
     * @return Boolean true if d1 is earlier than d2, false if either value is null, or if
     ...
     * @param d1 The first {@link DBDateTime} value to compare.
     * @param d2 The second {@link DBDateTime} value to compare.
     * @return Boolean true if d1 is later than d2, false if either value is null, or if
     ...
     * @return a null {@link DBDateTime} if either input is null;
     * the starting {@link DBDateTime} plus the specified number of nanoseconds, if the result is not too large for
     * a {@link DBDateTime}; or throws a {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the
     ...
     * @return a null {@link DBDateTime} if either input is null;
     * the starting {@link DBDateTime} minus the specified number of nanoseconds, if the result is not too negative for
     * a {@link DBDateTime}; or throws a {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the
     ...
     * @return a null {@link DBDateTime} if either input is null;
     * the starting {@link DBDateTime} plus the specified period, if the result is not too large for
     * a DBDateTime; or throws a {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the
     * resultant value is more than max long nanoseconds from Epoch.
     */
    public static DBDateTime plus(DBDateTime dateTime, DBPeriod period) {
        ...
        if (period.isPositive()) {
            return new DBDateTime(millisToNanos(dateTime.getJodaDateTime().plus(period.getJodaPeriod()).getMillis()) + dateTime.getNanosPartial());
        } else {
            return new DBDateTime(millisToNanos(dateTime.getJodaDateTime().minus(period.getJodaPeriod()).getMillis()) + dateTime.getNanosPartial());
        }
    }
     ...
     * @return a null {@link DBDateTime} if either input is null;
     * the starting {@link DBDateTime} minus the specified period, if the result is not too negative for
     * a {@link DBDateTime}; or throws a {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the
     * resultant value is more than min long nanoseconds from Epoch.
     */
     ...
     * @return {@link QueryConstants#NULL_LONG} if either input is null;
     * the long nanoseconds from Epoch value of the first {@link DBDateTime} minus d2, if the result is not out of range
     * for a long value; or throws a {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the
     * resultant value would be more than min long or max long nanoseconds from Epoch. <P>
     * Note that the subtraction is done based the nanosecond offsets of the two dates from Epoch, so,
     * if either date is before Epoch (negative offset), the result may be unexpected.</P>
     */
     ...
     * @return {@link QueryConstants#NULL_LONG} if either input is null;
     * a double value of the number of 365 day periods obtained from the first {@link DBDateTime} value minus d2,
     * if the intermediate value of nanoseconds difference between the two dates
     * is not out of range for a long value; or throws a {@link DBDateTimeOverflowException} if the
     * intermediate value would be more than min long or max long nanoseconds from Epoch. <P>
     * Note that the subtraction is done based the nanosecond offsets of the two dates from Epoch, so,
     * if either date is before Epoch (negative offset), the result may be unexpected.</P>
     * If the second value is greater than the first value, the result will be negative.
     */
     ...
     * @param end The second {@link DBDateTime}.
     * @return {@link QueryConstants#NULL_LONG} if either input is null;
     * a double value of the number of days obtained from the first {@link DBDateTime} value minus d2,
     * if the intermediate value of nanoseconds difference between the two dates
     * is not out of range for a long value; or throws a {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the
     * intermediate value would be more than min long or max long nanoseconds from Epoch. <P>
     * Note that the subtraction is done based the nanosecond offsets of the two dates from Epoch, so,
     * if either date is before Epoch (negative offset), the result may be unexpected.</P>
     * If the second value is greater than the first value, the result will be negative.
    ...
    /**
     * Returns a {@link DBDateTime} for the requested {@link DBDateTime} at midnight in the specified
     * time zone.
     ...
     * @return A null {@link DBDateTime} if either input is null, otherwise a {@link DBDateTime} representing
    ...
    public static DBDateTime dateAtMidnight(DBDateTime dateTime, DBTimeZone timeZone) {
        ...
        return new DBDateTime(millisToNanos(new DateMidnight(dateTime.getMillis(), timeZone.getTimeZone()).getMillis()) + dateTime.getNanosPartial());
    }
     ...
     * @return A {@link DBDateTime} rounded down to midnight in the New York time zone for the specified number of milliseconds
     * from Epoch.
     */
    @SuppressWarnings("WeakerAccess")
    ...
    /**
     * Returns a {@link DBDateTime} representing midnight in a selected time zone on the date specified by the a number of
     * milliseconds from Epoch.
     * @param millis A long value of the number of milliseconds from Epoch for which the {@link DBDateTime} is to be calculated.
     * @param timeZone {@link  DBTimeZone} for which the new value at midnight should be calculated.
     * @return A {@link DBDateTime} rounded down to midnight in the selected time zone for the specified number of milliseconds
     * from Epoch.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * <p>For values greater than one day, the output will start with dddT</p>
     * <p>For values with fractional seconds, the output will be trailed by .nnnnnnnnn</p>
     * <p>e.g. output may be dddThh:mm:ss.nnnnnnnnn or subsets of this.</p>
     */
    public static String format(long nanos) {
        ...
        buf.append(hours).append(':').append(StringUtils.pad(String.valueOf(minutes), 2, '0')).append(':').append(StringUtils.pad(String.valueOf(seconds), 2, '0'));
     ...
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of the day of the month represented by the {@link DBDateTime} when interpreted in the specified
     * time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * @param dateTime The {@link DBDateTime} for which to find the day of the month.
     * @return A {@link QueryConstants#NULL_INT} if the input is null, otherwise, an int value
     * of the day of the month represented by the {@link DBDateTime} when interpreted in the New York
     * time zone.
     */
    ...
    /**
     * Returns an int value of the day of the week for a {@link DBDateTime} in the specified time zone,
     * with 1 being Monday and 7 being Sunday.
     * @param dateTime The {@link DBDateTime} for which to find the day of the week.
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of the day of the week represented by the {@link DBDateTime} when interpreted in the specified
     * time zone.
     */
    ...
    /**
     * Returns an int value of the day of the week for a {@link DBDateTime} in the New York time zone,
     * with 1 being Monday and 7 being Sunday.
     * @param dateTime The {@link DBDateTime} for which to find the day of the week.
     * @return A {@link QueryConstants#NULL_INT} if the input is null, otherwise, an int value
     * of the day of the week represented by the {@link DBDateTime} when interpreted in the New York
     * time zone.
     */
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of the day of the year represented by the {@link DBDateTime} when interpreted in the specified
     * time zone.
     */
     ...
     * @param dateTime The {@link DBDateTime} for which to find the day of the year.
     * @return A {@link QueryConstants#NULL_INT} if the input is null, otherwise, an int value
     * of the day of the year represented by the {@link DBDateTime} when interpreted in the New York
     * time zone.
     */
    ...
    /**
     * Returns an int value of the hour of the day for a {@link DBDateTime} in the specified time zone.
     * The hour is on a 24 hour clock (0 - 23).
     * @param dateTime The {@link DBDateTime} for which to find the hour of the day.
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of the hour of the day represented by the {@link DBDateTime} when interpreted in the specified
     * time zone.
     */
    @SuppressWarnings("WeakerAccess")
    ...
    /**
     * Returns an int value of the hour of the day for a {@link DBDateTime} in the New York time zone.
     * The hour is on a 24 hour clock (0 - 23).
     * @param dateTime The {@link DBDateTime} for which to find the hour of the day.
     * @return A {@link QueryConstants#NULL_INT} if the input is null, otherwise, an int value
     * of the hour of the day represented by the {@link DBDateTime} when interpreted in the New York
     * time zone.
     */
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of milliseconds since midnight for the date/time represented by the {@link DBDateTime} when
    ...
    /**
     * Returns an int value of milliseconds since the top of the second for a {@link DBDateTime}
     * in the specified time zone.
     * @param dateTime The {@link DBDateTime} for which to find the milliseconds.
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of milliseconds since the top of the second for the date/time represented by the {@link DBDateTime}
     ...
     * of nanoseconds since midnight for the date/time represented by the {@link DBDateTime} when
     * interpreted in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * @param dateTime The {@link DBDateTime} for which to find the nanoseconds since midnight.
     * @return A {@link QueryConstants#NULL_LONG} if the input is null, otherwise, a long value
     * of nanoseconds since midnight for the date/time represented by the {@link DBDateTime} when
     * interpreted in the New York time zone.
     */
    ...
    /**
     * Returns a long value of nanoseconds since the top of the second for a {@link DBDateTime}
     * in the specified time zone.
     * @param dateTime The {@link DBDateTime} for which to find the nanoseconds.
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_LONG} if either input is null, otherwise, a long value
     * of nanoseconds since the top of the second for the date/time represented by the {@link DBDateTime}
     * when interpreted in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * provided {@code dateTime} in the specified time zone.
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of microseconds since the top of the millisecond for the date/time represented by the {@link DBDateTime}
     * when interpreted in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
    ...
    /**
     * Returns the number of microseconds that have elapsed since the start of the millisecond represented by the
     * provided {@code dateTime} in the New York time zone.
     * Nanoseconds are rounded, not dropped -- '20:41:39.123456700' has 457 micros, not 456.
     * @param dateTime The {@link DBDateTime} for which to find the microseconds.
     * @return A {@link QueryConstants#NULL_INT} if the input is null, otherwise, an int value
     * of microseconds since the top of the millisecond for the date/time represented by the {@link DBDateTime}
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of minutes since midnight for the date/time represented by the {@link DBDateTime} when
     * interpreted in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of minutes since the top of the hour for the date/time represented by the {@link DBDateTime} when
     * interpreted in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of the month for the date/time represented by the {@link DBDateTime} when
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of seconds since midnight for the date/time represented by the {@link DBDateTime} when
     * interpreted in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of seconds since the top of the minute for the date/time represented by the {@link DBDateTime} when
     * interpreted in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of the year for the date/time represented by the {@link DBDateTime} when
     * interpreted in the specified time zone.
     */
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return A {@link QueryConstants#NULL_INT} if either input is null, otherwise, an int value
     * of the two-digit year for the date/time represented by the {@link DBDateTime} when
     * interpreted in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * @param timeZone The {@link DBTimeZone} to use when interpreting the date/time.
     * @return 0.0 if either input is null, otherwise, a double value
     * containing the Excel double format representation of a {@link DBDateTime} in the specified time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     *
     * @param dateTime The {@link DBDateTime} to convert.
     * @return 0.0 if the input is null, otherwise, a double value
     * containing the Excel double format representation of a {@link DBDateTime} in the New York time zone.
     */
    @SuppressWarnings("WeakerAccess")
     ...
     * @return A {@link QueryConstants#NULL_LONG} if the input is null. Throws a {@link DBDateTimeOverflowException}
     ...
     * @return {@link QueryConstants#NULL_LONG} if the input is null, otherwise, a {@link DBDateTime}
    ...
    /**
     * Provides the current date/time, or, if a custom {@link #timeProvider} has been configured, provides the
     * current time according to the custom provider.
     * @return A {@link DBDateTime} of the current date and time from the system or from the configured alternate
     * time provider.
     */
    ...
    /**
     * Provides a String representing the current date in the New York time zone or, if a custom {@link #currentDateNyOverride}
     ...
     * {@link #currentDateNyOverride} value. If {@link #currentDateNyOverride} has not been set, this
     ...
     * Provides a String representing the previous business date in the New York time zone using the NYSE calendar, or,
     * if a custom {@link #lastBusinessDayNyOverride} has been set, the date provided by that override.
     * @param currentTimeMillis The current date/time in milliseconds from Epoch to be used when determining
    ...
    /**
     * Returns the number of milliseconds from Epoch for midnight at the beginning of the next day in the New York
     * time zone relative to the date represented by a passed milliseconds from Epoch date/time.
     * @param currentTimeMillis A long value of milliseconds from Epoch which is the date/time from which the next
     *                          New York time zone midnight value should be calculated.
     * @return A long of milliseconds from Epoch for midnight at the beginning of the next day in the New York
     * time zone.
     */
    ...
    private static class CachedCurrentDate extends CachedDate {
        ...
        void update(final long currentTimeMillis) {
            ...
            valueExpirationTimeMillis = new DateTime(currentTimeMillis, timeZone.getTimeZone()).withFieldAdded(DurationFieldType.days(), 1).withTimeAtStartOfDay().getMillis();
        }
    }

    private static class CachedDateKey<CACHED_DATE_TYPE extends CachedDate> extends KeyedObjectKey.Basic<DBTimeZone, CACHED_DATE_TYPE> {
    ...
    private static final KeyedObjectHashMap<DBTimeZone, CachedCurrentDate> cachedCurrentDates = new KeyedObjectHashMap<>(new CachedDateKey<CachedCurrentDate>());
     ...
     * @return A DBDateTime for {@code nanos}, or {@code null} if {@code nanos}
     * is equal to {@link QueryConstants#NULL_LONG NULL_LONG}.
     */
     ...
     * @return null, if the input is equal to {@link QueryConstants#NULL_LONG}, otherwise a {@link DBDateTime} based
    ...
    /**
     * Returns a {@link DBDateTime} value based on a starting value and a {@link DBPeriod} to add to it,
     ...
     * @return a null {@link DBDateTime} if either original or period are null;
     * the starting {@link DBDateTime} plus the specified period, if the result is not too large for
     * a DBDateTime and does not exceed the cap value; the cap value if this is less than offset plus period.
     * Throws a {@link DBDateTimeOverflowException DBDateTimeOverflowException} if the
     * resultant value is more than max long nanoseconds from Epoch.
     */
    ...
    /**
     * Returns a {@link DBDateTime} value, which is at the starting (lower) end of a time range defined
     ...
     * @return Null if either input is null, otherwise a {@link DBDateTime} representing the start of the
     * window.
     */
     ...
     * @param dateTime The {@link DBDateTime} for which to evaluate the start of the containing window.
     * @param intervalNanos The size of the window in nanoseconds.
     * @param offset The window start offset in nanoseconds.  For example, a value of MINUTE would offset all windows by one minute.
     * @return Null if either input is null, otherwise a {@link DBDateTime} representing the start of the
     * window.
     */
    ...
    /**
     * Returns a {@link DBDateTime} value, which is at the ending (upper) end of a time range defined
     * by the interval nanoseconds. For example, a 5*MINUTE intervalNanos value would return the date/time value
     ...
     * @param intervalNanos The size of the window in nanoseconds.
     * @return Null if either input is null, otherwise a {@link DBDateTime} representing the end of the
     * window.
     */
    ...
    public static DBDateTime upperBin(DBDateTime dateTime, long intervalNanos, long offset) {
        if (dateTime == null || intervalNanos == io.deephaven.util.QueryConstants.NULL_LONG || offset == io.deephaven.util.QueryConstants.NULL_LONG ) {
            return null;
        }
     ...
     * @return A {@link Result} object, which includes the converted formula string,
    ...
    public static Result convertExpression(String formula) throws Exception {    // TODO: Why throw Exception?
        ...
        while (matcher.find()) {
            ...
            if (convertDateTimeQuiet(s) != null) {
                ...
                instanceVariablesString.append("        private DBDateTime _date").append(dateTimeIndex).append("=DBTimeUtils.convertDateTime(\"").
                ...
                instanceVariablesString.append("        private java.time.LocalDate _localDate").append(localDateIndex).append("=DBTimeUtils.convertDate(\"").
                        append(formula, matcher.start() + 1, matcher.end() - 1).append("\");\n");
                ...
                instanceVariablesString.append("        private long _time").append(timeIndex).append("=DBTimeUtils.convertTime(\"").
                        append(formula, matcher.start() + 1, matcher.end() - 1).append("\");\n");
                ...
                instanceVariablesString.append("        private DBPeriod _period").append(periodIndex).append("=DBTimeUtils.convertPeriod(\"").
                        append(formula, matcher.start() + 1, matcher.end() - 1).append("\");\n");
                ...
                instanceVariablesString.append("        private java.time.LocalTime _localTime").append(timeIndex).append("=DBTimeUtils.convertLocalTime(\"").
                        append(formula, matcher.start() + 1, matcher.end() - 1).append("\");\n");
     ...
     * <p>yyyy-MM-ddThh:mm:ss[.nnnnnnnnn] TZ for date/time values</p>
     * <p>hh:mm:ss[.nnnnnnnnn] for time values</p>
     * <p>Period Strings in the form of numbertype, e.g. 1W for one week, and Tnumbertype for times, e.g. T1M for one
     * minute</p>
     * @param formula The String to be evaluated and converted. Optionally, but preferred, enclosed in straight single
     *                ticks.
     * @return A long value representing an Epoch offset in nanoseconds for a time or date/time, or a duration
     * in nanoseconds for a period. Throws {@link DBDateTimeOverflowException} if the resultant value would be
    ...
    /**
     * Attempt to convert the given string to a LocalDate. This should <b>not</b> accept dates with times, as we want those
     ...
     *
     * @param s String to be converted, usually in the form yyyy-MM-ddThh:mm:ss and with optional sub-seconds
    ...
    /**
     * Converts a String time to nanoseconds from Epoch. The format for the String is: <p>hh:mm:ss[.nnnnnnnnn].
     * @param s The String to be evaluated and converted.
     * @return A long value representing an Epoch offset in nanoseconds. Throws {@link RuntimeException} if the
    ...
    private static int extractThreeDigitNum(String s, int startIndex) {
        return (s.charAt(startIndex) - '0') * 100 + (s.charAt(startIndex + 1) - '0') * 10 + (s.charAt(startIndex + 2) - '0');
    }

    private static int extractFourDigitNum(String s, int startIndex) {
        return (s.charAt(startIndex) - '0') * 1000 + (s.charAt(startIndex + 1) - '0') * 100 + (s.charAt(startIndex + 2) - '0') * 10 + (s.charAt(startIndex + 3) - '0');
    }
    ...
    public static LocalTime convertLocalTimeQuiet(String s) {
        try {
            // private static final Pattern LOCAL_TIME_PATTERN = Pattern.compile("([0-9][0-9]):?([0-9][0-9])?:?([0-9][0-9])?(\\.([0-9]{1,9}))?");
    ...
    /* This version assumes you know what date it is and that the format is correct and just want the time, so we can save time
   (e.g. 2010-09-02T08:17:17.502-0400)
    ...
    */
    ...
    /**
     * Converts a String of digits of any length to a nanoseconds long value. Will ignore anything longer than
     * 9 digits, and will throw a NumberFormatException if any non-numeric character is found. Strings shorter
     * than 9 digits will be interpreted as sub-second values to the right of the decimal point.
     *
     ...
     * @param s The String to convert.
     * @return {@link QueryConstants#NULL_LONG} if the String cannot be parsed, otherwise long nanoseconds offset from Epoch.
     */
        ...
        try {
            if (TIME_AND_DURATION_PATTERN.matcher(s).matches()) {
                ...
                if (tokens.length == 2) {   //hh:mm
                    return multiplier * (1000000000L * (3600 * Integer.parseInt(tokens[0]) + 60 * Integer.parseInt(tokens[1])) + dayNanos + subsecondNanos);
                } else if (tokens.length == 3) {   //hh:mm:ss
                    return multiplier * (1000000000L * (3600 * Integer.parseInt(tokens[0]) + 60 * Integer.parseInt(tokens[1]) + Integer.parseInt(tokens[2])) + dayNanos + subsecondNanos);
                }
            }
        } catch (Exception e) {
            //shouldn't get here too often, but somehow something snuck through.  we'll just return null below...
     ...
     * @return A {@link ZonedDateTime} using the default time zone for the session
     * as indicated by {@link DBTimeZone#TZ_DEFAULT}.
     */
     ...
     * @throws DBDateTimeOverflowException if the input is out of the range for a {@link DBDateTime},
     * otherwise, a {@link DBDateTime} version of the input.
     */
    ...
    /**
     * A type of RuntimeException thrown when operations resulting in {@link DBDateTime} values would
     ...
     */
    public static String getPartitionFromTimestampMillis(@NotNull final DateTimeFormatter dateTimeFormatter, final long timestampMillis) {
     ...
     * @return the formatted date
     */
    public static String getPartitionFromTimestampMicros(@NotNull final DateTimeFormatter dateTimeFormatter, final long timestampMicros) {
     ...
     * @return the formatted date
     */
    public static String getPartitionFromTimestampNanos(@NotNull final DateTimeFormatter dateTimeFormatter, final long timestampNanos) {
     ...
     * @return the formatted date
     */
    public static String getPartitionFromTimestampSeconds(@NotNull final DateTimeFormatter dateTimeFormatter, final long timestampSeconds) {
