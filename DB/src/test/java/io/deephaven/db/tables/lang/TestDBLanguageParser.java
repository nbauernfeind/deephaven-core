...
public class TestDBLanguageParser extends BaseArrayTestCase {
    ...
    /**
     * Test casts. See
     * <a href="https://docs.oracle.com/javase/specs/jls/se8/html/jls-5.html#jls-5.5">Chapter 5, Conversions and Contexts</a>,
     * in the java language specification for more info.
     *
    ...
    public void testMiscellaneousCasts() throws Exception {
        ...
        expression = "(double)myIntObj"; // requires separate casts for unboxing & widening (see notes at testBoxedToPrimitiveCasts, or JLS)
        resultExpression = "doubleCast(intCast(myIntObj))";
        check(expression, resultExpression, double.class, new String[]{"myIntObj"});
    ...
    /**
     * This is an older version of {@link #testPrimitiveVariableCasts()}, operating with
    ...
    public void testPrimitiveLiteralCasts() throws Exception {
        ...
        /* Test casting from each possible numeric literal type (and char) to each of
         * the other numeric types (and char).
         *
         * When casting to a primitive type, we replace the cast with a function call
         * (e.g "(int)foo" to "intCast(foo)")
         *
     ...
     * <p>
     * When casting from boxed types to primitive types, the target type must be at least as wide as the primitive
     * type that the source (boxed) type represents. Thus there are two kinds of conversions from boxed types
     * to primitive types:
     * <p>
     * 1. Unboxing-only conversions (e.g. Integer to int)
     * 2. Unboxing and widening conversions (e.g. Integer to double)
     * <p>
     * In the latter case, the language must explicitly cast an Integer to an int *before* casting it to a double. This
     * follow's the language specification: when permitted, a non-identity conversion from a boxed type to a primitive type
     * consists of an unboxing conversion followed by a widening conversion.
     * <p>
     * For example, this code:
     * {@code (double) new Integer(42) }
     * Should be parsed into:
     * {@code doubleCast(intCast(new Integer(42))) }
     * <p>
     * Otherwise, the compiler would see {@code doubleCast()} with an {@code Integer} argument,
     * and rightly decide that {@code doubleCast(Object)} is a better choice than {@code doubleCast(int)}. But then
     * we wind up running:
    ...
    public void testBoxedToPrimitiveCasts() {
        ...
        for (int i = 0; i < io.deephaven.util.type.TypeUtils.BOXED_TYPES.size(); i++) {
            ...
            final String unboxedTypeName = io.deephaven.util.type.TypeUtils.getUnboxedType(boxedType).getName(); // the name of the primitive type that this boxed type represents
            final String boxedTypeTestVarName = "my" + Character.toUpperCase(unboxedTypeName.charAt(0)) + unboxedTypeName.substring(1) + "Obj";
            ...
            for (int j = 0; j < io.deephaven.util.type.TypeUtils.PRIMITIVE_TYPES.size(); j++) {
                ...
                if (primitiveType == boolean.class) {
                    ...
                    resultExpression = primitiveTypeName + "Cast(" + unboxedTypeName + "Cast(" + boxedTypeTestVarName + "))";
                }
                ...
                try {
                    ...
                    /* Ensure we fail on conversions that the JLS disallows. Such as:
                     * 1) Trying to convert to a primitive type that is not sufficient to store
                     *    the boxed type's data (i.e. j < i). The JLS does not permit such a conversion.
                     * 2) Trying to unbox anything other than a Character to a char. (However, char can be
                     *    cast to wider (int, long, float, double).
                     * 3) Casting a Boolean to any primitive besides bool, or casting anything besides a
                     *    Boolean to bool.
                     *
                    ...
                    throw new RuntimeException("Failed testing cast of " + boxedType.getName() + " to " + primitiveType.getName() + " (i=" + i + ", j=" + j + ')', ex);
                }
            }
        }
    }
    ...
    public void testArrayOperatorOverloading() throws Exception{
        ...
        check(expression, resultExpression, new boolean[0].getClass(), new String[]{"myTestClass", "myTestClassArray"});
...
//    }
    ...
    public void testImplicitConversion() throws Exception {
        ...
        String resultExpression="testImplicitConversion1(new double[] { doubleCast(myInt), myDouble, doubleCast(myLong), doubleCast(myInt), myDouble, doubleCast(myLong) })";
...
//        resultExpression="testImplicitConversion3(myDoubleArray)"; // we should *not* convert from DbDoubleArray to Object[]!
...
//        expression="testImplicitConversion5(myDBArray)"; // TODO: This test fails (declared arg type is "DbArrayBase...")
...
//        check(expression, resultExpression, new Object[0].getClass(), new String[]{"myDBArray"});
    ...
    /**
     * Test calling the default methods from {@link Object}. (In the past, these
     * were not recognized on interfaces.)
     */
    ...
    public void testFieldAccess() throws Exception {
        ...
        check(expression, resultExpression, DBLanguageParserDummyClass.InnerClass.InnerInnerClass.class, new String[]{"myDummyClass"});
    ...
    public void testBadFieldAccess() throws Exception {
        ...
        try {
            ...
            try {
                ...
                if (
                        !ex.getMessage().contains("Scope      : myDummyInnerClass") ||
            ...
            try {
                ...
                if (
                        !ex.getMessage().contains("Scope      : DBLanguageParserDummyClass.StaticNestedClass") ||
                        !ex.getMessage().contains("Field Name : staticVarThatDoesNotExist")) {
                    fail("Useless exception message!\nOriginal exception:\n" + ExceptionUtils.getStackTrace(ex));
                }
            }
            ...
            try {
                ...
                if (
                        !ex.getMessage().contains("Scope      : myDummyNonExistentInnerClass") ||
            ...
            try {
                ...
                if (
                        !ex.getMessage().contains("Scope      : DBLanguageParserNonExistentDummyClass.StaticNestedClass") ||
                        !ex.getMessage().contains("Field Name : staticVar")) {
                    fail("Useless exception message!\nOriginal exception:\n" + ExceptionUtils.getStackTrace(ex));
                }
            }
        ...
        /* Also test within a method call. This is essentially the case that prompted the fix. The actual issue
           with the expression is that the enclosing class name is omitted when trying to access the
           nested enum (NestedEnum.ONE), but the user experience was poor because the exception was very unclear.

           There is a test of the proper expression in testComplexExpressions().
         */
            try {
                expression = "io.deephaven.db.tables.lang.DBLanguageParserDummyClass.interpolate(myDoubleDBArray.toArray(),myDoubleDBArray.toArray(),new double[]{myDouble},io.deephaven.db.tables.lang.NestedEnum.ONE,false)[0]";
                resultExpression = "io.deephaven.db.tables.lang.DBLanguageParserDummyClass.interpolate(myDoubleDBArray.toArray(),myDoubleDBArray.toArray(),new double[]{myDouble},io.deephaven.db.tables.lang.NestedEnum.ONE,false)[0]";
                check(expression, resultExpression, String.class, new String[]{});
                fail("Should have thrown a DBLanguageParser.QueryLanguageParseException");
            } catch (DBLanguageParser.QueryLanguageParseException ex) {
                if (
                        !ex.getMessage().contains("Scope      : io.deephaven.db.tables.lang.NestedEnum") ||
    ...
    public void testEnums() throws Exception {
        ...
        check(expression, resultExpression, DBLanguageParserDummyClass.SubclassOfDBLanguageParserDummyClass.EnumInInterface.class, new String[]{});
        ...
        expression="DBLanguageParserDummyClass.functionWithEnumAsArgs(DBLanguageParserDummyInterface.AnEnum.THING_ONE)";
        resultExpression="DBLanguageParserDummyClass.functionWithEnumAsArgs(DBLanguageParserDummyInterface.AnEnum.THING_ONE)";
        check(expression, resultExpression, int.class, new String[]{});

        expression="DBLanguageParserDummyClass.functionWithEnumVarArgs(myEnumValue, DBLanguageParserDummyEnum.ONE)";
        resultExpression="DBLanguageParserDummyClass.functionWithEnumVarArgs(myEnumValue, DBLanguageParserDummyEnum.ONE)";
    ...
    public void testEqualsConversion() throws Exception {
        DBLanguageParser.Result result = new DBLanguageParser("1==1", null, null, staticImports, null, null).getResult();
    ...
    /**
     * In order to support the null values defined in {@link QueryConstants}, language parser converts
     * the equality and relational operators into method calls.
     */
    ...
    public void testArraysAsArguments() throws Exception {
        ...
        resultExpression="DBLanguageParserDummyClass.arrayOnlyFunction(ArrayUtils.nullSafeDbArrayToArray(myIntDBArray))";
        check(expression, resultExpression, long.class, new String[]{"myIntDBArray"});
    ...
    public void testDBArrayUnboxing() throws Exception{
        ...
        resultExpression="eqArray(ArrayUtils.nullSafeDbArrayToArray(myIntDBArray), ArrayUtils.nullSafeDbArrayToArray(myIntDBArray))";
    ...
    public void testInnerClasses() throws Exception{
        DBLanguageParser.Result result = new DBLanguageParser("io.deephaven.db.tables.lang.TestDBLanguageParser.InnerEnum.YEAH!=null", null, null, staticImports, null, null).getResult();
        assertEquals("!eq(io.deephaven.db.tables.lang.TestDBLanguageParser.InnerEnum.YEAH, null)", result.getConvertedExpression());
    }

    public void testComplexExpressions() throws Exception {
        String expression="java.util.stream.Stream.of(new String[] { `a`, `b`, `c`, myInt > 0 ? myString=Double.toString(myDouble) ? `1` : `2` : new DBLanguageParserDummyClass().toString() }).count()";
        String resultExpression="java.util.stream.Stream.of(new String[] { \"a\", \"b\", \"c\", greater(myInt, 0) ? eq(myString, Double.toString(myDouble)) ? \"1\" : \"2\" : new DBLanguageParserDummyClass().toString() }).count()";
        ...
        resultExpression="eq(myDummyClass.innerClassInstance.staticVar, 1_000_000L)" +
                ...
                " : myIntArray";
        check(expression, resultExpression, int[].class, new String[]{"myDouble", "myDummyClass", "myInt", "myIntArray", "myString"});

        // This comes from a strategy query:
        expression = "min( abs(ExampleQuantity), (int)round(min(ExampleQuantity2, (`String1`.equals(ExampleStr) ? max(-ExampleQuantity3,0d) : max(ExampleQuantity3,0d))/ExampleQuantity4)))";
        ...
        check(expression, resultExpression, int.class, new String[]{"ExampleQuantity", "ExampleQuantity2", "ExampleQuantity3", "ExampleQuantity4", "ExampleStr" });

        // There is a test for an erroneous version of this expression in testBadFieldAccess():
        expression = "io.deephaven.db.tables.lang.DBLanguageParserDummyClass.interpolate(myDoubleDBArray.toArray(),myDoubleDBArray.toArray(),new double[]{myDouble},io.deephaven.db.tables.lang.DBLanguageParserDummyClass.NestedEnum.ONE,false)[0]";
        resultExpression = "io.deephaven.db.tables.lang.DBLanguageParserDummyClass.interpolate(myDoubleDBArray.toArray(), myDoubleDBArray.toArray(), new double[] { myDouble }, io.deephaven.db.tables.lang.DBLanguageParserDummyClass.NestedEnum.ONE, false)[0]";
        ...
        expression = "DBLanguageParserDummyClass.interpolate(myDoubleDBArray,myDoubleDBArray,new double[]{myDouble},DBLanguageParserDummyClass.NestedEnum.ONE,false)[0]";
        resultExpression = "DBLanguageParserDummyClass.interpolate(ArrayUtils.nullSafeDbArrayToArray(myDoubleDBArray), ArrayUtils.nullSafeDbArrayToArray(myDoubleDBArray), new double[] { myDouble }, DBLanguageParserDummyClass.NestedEnum.ONE, false)[0]";
        check(expression, resultExpression, double.class, new String[]{"myDouble", "myDoubleDBArray"});
    }
    ...
    public void testIsWideningPrimitiveConversion() {
        {
            Require.eqFalse(isWideningPrimitiveConversion(byte.class, byte.class), "isWideningPrimitiveConversion(byte.class, byte.class)");
            Require.eqFalse(isWideningPrimitiveConversion(byte.class, char.class), "isWideningPrimitiveConversion(byte.class, char.class)");

            Require.eqTrue(isWideningPrimitiveConversion(byte.class, short.class), "isWideningPrimitiveConversion(byte.class, short.class)");
            Require.eqTrue(isWideningPrimitiveConversion(byte.class, int.class), "isWideningPrimitiveConversion(byte.class, int.class)");
            Require.eqTrue(isWideningPrimitiveConversion(byte.class, long.class), "isWideningPrimitiveConversion(byte.class, long.class)");
            Require.eqTrue(isWideningPrimitiveConversion(byte.class, float.class), "isWideningPrimitiveConversion(byte.class, float.class)");
            Require.eqTrue(isWideningPrimitiveConversion(byte.class, double.class), "isWideningPrimitiveConversion(byte.class, double.class)");
        }


        {
            Require.eqFalse(isWideningPrimitiveConversion(short.class, byte.class), "isWideningPrimitiveConversion(short.class, byte.class)");
            Require.eqFalse(isWideningPrimitiveConversion(short.class, short.class), "isWideningPrimitiveConversion(short.class, short.class)");
            Require.eqFalse(isWideningPrimitiveConversion(short.class, char.class), "isWideningPrimitiveConversion(short.class, char.class)");

            Require.eqTrue(isWideningPrimitiveConversion(short.class, int.class), "isWideningPrimitiveConversion(short.class, int.class)");
            Require.eqTrue(isWideningPrimitiveConversion(short.class, long.class), "isWideningPrimitiveConversion(short.class, long.class)");
            Require.eqTrue(isWideningPrimitiveConversion(short.class, float.class), "isWideningPrimitiveConversion(short.class, float.class)");
            Require.eqTrue(isWideningPrimitiveConversion(short.class, double.class), "isWideningPrimitiveConversion(short.class, double.class)");
        }

        {
            Require.eqFalse(isWideningPrimitiveConversion(char.class, byte.class), "isWideningPrimitiveConversion(char.class, byte.class)");
            Require.eqFalse(isWideningPrimitiveConversion(char.class, short.class), "isWideningPrimitiveConversion(char.class, short.class)");
            Require.eqFalse(isWideningPrimitiveConversion(char.class, char.class), "isWideningPrimitiveConversion(char.class, char.class)");

            Require.eqTrue(isWideningPrimitiveConversion(char.class, int.class), "isWideningPrimitiveConversion(char.class, int.class)");
            Require.eqTrue(isWideningPrimitiveConversion(char.class, long.class), "isWideningPrimitiveConversion(char.class, long.class)");
            Require.eqTrue(isWideningPrimitiveConversion(char.class, float.class), "isWideningPrimitiveConversion(char.class, float.class)");
            Require.eqTrue(isWideningPrimitiveConversion(char.class, double.class), "isWideningPrimitiveConversion(char.class, double.class)");
        }

        {
            Require.eqFalse(isWideningPrimitiveConversion(int.class, byte.class), "isWideningPrimitiveConversion(int.class, byte.class)");
            Require.eqFalse(isWideningPrimitiveConversion(int.class, short.class), "isWideningPrimitiveConversion(int.class, short.class)");
            Require.eqFalse(isWideningPrimitiveConversion(int.class, char.class), "isWideningPrimitiveConversion(int.class, char.class)");
            Require.eqFalse(isWideningPrimitiveConversion(int.class, int.class), "isWideningPrimitiveConversion(int.class, int.class)");

            Require.eqTrue(isWideningPrimitiveConversion(int.class, long.class), "isWideningPrimitiveConversion(int.class, long.class)");
            Require.eqTrue(isWideningPrimitiveConversion(int.class, float.class), "isWideningPrimitiveConversion(int.class, float.class)");
            Require.eqTrue(isWideningPrimitiveConversion(int.class, double.class), "isWideningPrimitiveConversion(int.class, double.class)");
        }

        {
            Require.eqFalse(isWideningPrimitiveConversion(long.class, byte.class), "isWideningPrimitiveConversion(long.class, byte.class)");
            Require.eqFalse(isWideningPrimitiveConversion(long.class, short.class), "isWideningPrimitiveConversion(long.class, short.class)");
            Require.eqFalse(isWideningPrimitiveConversion(long.class, char.class), "isWideningPrimitiveConversion(long.class, char.class)");
            Require.eqFalse(isWideningPrimitiveConversion(long.class, int.class), "isWideningPrimitiveConversion(long.class, int.class)");
            Require.eqFalse(isWideningPrimitiveConversion(long.class, long.class), "isWideningPrimitiveConversion(long.class, long.class)");


            Require.eqTrue(isWideningPrimitiveConversion(long.class, float.class), "isWideningPrimitiveConversion(long.class, float.class)");
            Require.eqTrue(isWideningPrimitiveConversion(long.class, double.class), "isWideningPrimitiveConversion(long.class, double.class)");
        }


        {
            Require.eqFalse(isWideningPrimitiveConversion(float.class, byte.class), "isWideningPrimitiveConversion(float.class, byte.class)");
            Require.eqFalse(isWideningPrimitiveConversion(float.class, short.class), "isWideningPrimitiveConversion(float.class, short.class)");
            Require.eqFalse(isWideningPrimitiveConversion(float.class, char.class), "isWideningPrimitiveConversion(float.class, char.class)");
            Require.eqFalse(isWideningPrimitiveConversion(float.class, int.class), "isWideningPrimitiveConversion(float.class, int.class)");
            Require.eqFalse(isWideningPrimitiveConversion(float.class, long.class), "isWideningPrimitiveConversion(float.class, long.class)");

            Require.eqTrue(isWideningPrimitiveConversion(float.class, double.class), "isWideningPrimitiveConversion(float.class, double.class)");
        }

        {
            Require.eqFalse(isWideningPrimitiveConversion(double.class, byte.class), "isWideningPrimitiveConversion(double.class, byte.class)");
            Require.eqFalse(isWideningPrimitiveConversion(double.class, short.class), "isWideningPrimitiveConversion(double.class, short.class)");
            Require.eqFalse(isWideningPrimitiveConversion(double.class, char.class), "isWideningPrimitiveConversion(double.class, char.class)");
            Require.eqFalse(isWideningPrimitiveConversion(double.class, int.class), "isWideningPrimitiveConversion(double.class, int.class)");
            Require.eqFalse(isWideningPrimitiveConversion(double.class, long.class), "isWideningPrimitiveConversion(double.class, long.class)");
            Require.eqFalse(isWideningPrimitiveConversion(double.class, float.class), "isWideningPrimitiveConversion(double.class, float.class)");
            Require.eqFalse(isWideningPrimitiveConversion(double.class, double.class), "isWideningPrimitiveConversion(double.class, long.class)");
        }

        {
            Require.eqFalse(isWideningPrimitiveConversion(boolean.class, byte.class), "isWideningPrimitiveConversion(boolean.class, byte.class)");
            Require.eqFalse(isWideningPrimitiveConversion(boolean.class, short.class), "isWideningPrimitiveConversion(boolean.class, short.class)");
            Require.eqFalse(isWideningPrimitiveConversion(boolean.class, char.class), "isWideningPrimitiveConversion(boolean.class, char.class)");
            Require.eqFalse(isWideningPrimitiveConversion(boolean.class, int.class), "isWideningPrimitiveConversion(boolean.class, int.class)");
            Require.eqFalse(isWideningPrimitiveConversion(boolean.class, long.class), "isWideningPrimitiveConversion(boolean.class, long.class)");
            Require.eqFalse(isWideningPrimitiveConversion(boolean.class, float.class), "isWideningPrimitiveConversion(boolean.class, float.class)");
            Require.eqFalse(isWideningPrimitiveConversion(boolean.class, double.class), "isWideningPrimitiveConversion(boolean.class, double.class)");
            Require.eqFalse(isWideningPrimitiveConversion(boolean.class, boolean.class), "isWideningPrimitiveConversion(boolean.class, boolean.class)");
        }
    }
    ...
    public void testClassExpr() throws Exception {
        ...
        check(expression, resultExpression, DBLanguageParserDummyClass.InnerClass.class, new String[]{});

        expression="DBLanguageParserDummyInterface.class";
        resultExpression="DBLanguageParserDummyInterface.class";
        check(expression, resultExpression, DBLanguageParserDummyInterface.class, new String[]{});
    }
    ...
    private void check(String expression, String resultExpression, Class<?> resultType, String[] resultVarsUsed) throws Exception {
        DBLanguageParser.Result result = new DBLanguageParser(expression, packageImports, classImports, staticImports, variables, variableParameterizedTypes).getResult();
